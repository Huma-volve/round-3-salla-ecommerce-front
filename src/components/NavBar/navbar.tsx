    ...
    <div className="w-full overflow-x-hidden max-w-[100vw]">
      <header className="w-full border-b text-sm font-light">
          ...
          <div className="lg:hidden fixed top-0 right-0 w-[90%] h-full bg-white shadow-md z-50 p-6 flex flex-col overflow-y-auto">
            ...
            {isLoggedIn ? (
              <div className="flex items-center gap-2 pt-8 mb-4">
                ...
                />
                <span className="font-regular text-gray-600 font-bold">
                  {userName}
                </span>
              </div>
            ) : (
            ...
            <ul className="flex flex-col gap-3 text-sm text-gray-700 font-regular mt-4">
              ...
              <Link to="/products" className="text-gray-500 font-bold ">
                سياسة الاستبدال أو الاسترجاع
              </Link>
              <Link to="/products" className="text-gray-500 font-bold ">
                المفضلة
              </Link>
            ...
            <div className="mt-6 ">
              <nav className="flex flex-col gap-4">
                <Link
                  to="/productDetailUpper"
                  className="  font-bold text-center items-center gap-1 text-accent-primary border border-accent-primary px-4 py-1 rounded  "
                >
                  كل المنتجات
                </Link>
                <Link
                ...
                <Link
                  to="/signin"
                  className="text-center border-2  flex items-center gap-2 text-lg px-4 py-2 rounded-full bg-accent-primary text-white font-regular font-bold "
                >
          ...
          <div className="hidden lg:flex items-center gap-4 flex-shrink-0">
            <div className="text-right relative">
              <div className="flex items-center gap-1">
                <User
                  size={20}
                  className="text-gray-700 rounded-full bg-gray-200"
                />
                <div>
        ...
        <nav className="hidden lg:flex bg-black text-white px-4 py-2 gap-6 justify-center text-sm">
          ...
          >
            كل المنتجات <ChevronDown size={16} />
          </Link>
          <Link
            to="/products"
            className="cursor-pointer flex items-center gap-1"
          >
            الكترونيات <ChevronDown size={16} />
          </Link>
          <Link
            to="/products"
            className="cursor-pointer flex items-center gap-1"
          >
            موبايلات <ChevronDown size={16} />
          </Link>
          <Link
            to="/products"
            className="cursor-pointer flex items-center gap-1"
          >
            كاميرات مراقبة <ChevronDown size={16} />
          </Link>
          <Link
            to="/products"
            className="cursor-pointer flex items-center gap-1"
          >
            سماعات رقمية <ChevronDown size={16} />
          </Link>
          <Link
            to="/products"
            className="cursor-pointer flex items-center gap-1"
          >
            كاميرات رقمية <ChevronDown size={16} />
          </Link>
          <Link
            to="/products"
            className="cursor-pointer flex items-center gap-1"
          >
            اكسسوارات <ChevronDown size={16} />
          </Link>
          <Link
            to="/products"
            className="cursor-pointer flex items-center gap-1"
          >
            شاشات كمبيوتر <ChevronDown size={16} />
          </Link>
          <Link
            to="/products"
            className="cursor-pointer flex items-center gap-1"
